const tagBtns = document.querySelectorAll('.bjs-tipDetail-tag-btn');

tagBtns.forEach((tagBtn) => {
  tagBtn.addEventListener('click', function () {
    // 모든 버튼에서 bjs-ask-select 클래스 제거
    tagBtns.forEach((btn) => btn.classList.remove('bjs-tipDetail-select'));

    // 클릭된 버튼에 bjs-ask-select 클래스 추가
    tagBtn.classList.add('bjs-tipDetail-select');
  });
});
...
function addComment() {
  ...
  if (commentText !== '') {
    ...
    commentArea.classList.add('bjs-tipDetail-comment-show');

    // 댓글 작성자
    const commmentAuthor = document.createElement('span');
    commmentAuthor.classList.add('bjs-tipDetail-commnet-author');
    //bjs-tipDetail-commnet-author
    commmentAuthor.textContent = "3inch 임초현🏅"; // 나중에 불러오는거 여기 추가

    // 댓글 (진짜)
    const comment = document.createElement('div');
    comment.classList.add('bjs-tipDetail-comment-section');
    comment.textContent = commentText;

    // 작성일자, 조회수, 모니터 담은 박스
    const commentInfo = document.createElement('div');
    commentInfo.classList.add('bjs-tipDetail-comment-info');

    // 작성일자
    const commentDate = document.createElement('span');
    commentDate.classList.add('bjs-tipDetail-comment-date');
    commentDate.textContent = "1시간 전" // 나중에 불러오는거 여기 추가

    // 모니터 수, 이미지 박스
    const commentMonitor = document.createElement('span');
    commentMonitor.classList.add('bjs-tipDetail-comment-monitor');


    // 모니터 수
    const commentMonitorNum = document.createElement('span');
    commentMonitorNum.classList.add('bjs-tipDetail-comment-monitor-num');
    ...
    commentdelete.addEventListener('click', function () {
      if(comment.classList.contains("bjs-tipDetail-delete-comment-style")){
        alert('이미 삭제된 댓글입니다.');
      }else if (confirm('댓글을 삭제하시겠습니까?')) {
        comment.textContent = '삭제된 댓글입니다.';
        comment.classList.add('bjs-tipDetail-delete-comment-style');
        // 모니터 수 - 변경
        commentMonitorNum.textContent = '-';
      }
    });
    commentdelete.classList.add('bjs-tipDetail-comment-delete');
    ...
    // 코드 수정 ---------------------------------------------------
    commentmodify.classList.add('bjs-tipDetail-comment-modify');
